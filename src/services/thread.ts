  ...
  for (const groupId of state.boundGroups) {
    try {
      ...
      if (thread.img && thread.ext) {
        ...
        const imageUrl =
          `${config.xdnmbImageBase}/image/${thread.img}${thread.ext}`;
        try {
          ...
          const messageWithImageLink =
            `<a href="${imageUrl}">${thread.img}</a>\n${initialMessage}`;
    ...
    for (const [threadId, threadState] of Object.entries(allThreads)) {
      try {
        if (!(await shouldCheckThread(threadId, threadState))) {
          const lastNewReplyAt = threadState.lastNewReplyAt ??
            threadState.lastCheck;
          const daysSinceNewReply =
            (Date.now() - new Date(lastNewReplyAt).getTime()) /
            (1000 * 60 * 60 * 24);
          console.log(
            `⏭️  Skipping inactive thread ${threadId} (${
              Math.floor(daysSinceNewReply)
  ...
  for (const binding of threadState.bindings) {
    ...
    for (const reply of replies) {
      ...
      for (let i = 0; i < messageChunks.length; i++) {
        ...
        const batchLength = currentLength +
          chunkLength +
        ...
        if (isImage && i === messageChunks.length - 1) {
          ...
          currentLength += chunkLength +
            (currentBatch.length > 1 ? SEPARATOR.length : 0);
        }
      }
    }
